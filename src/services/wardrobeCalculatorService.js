...
function calculateEstimate({ length, height, type, package: packageType }) {
    ...
    if (!length || !height || !type || !packageType) {
        throw new Error('Missing required wardrobe configuration: length, height, type, and package are required');
    }
    ...
    if (!['basic', 'premium', 'luxury'].includes(packageType)) {
        throw new Error(`Invalid package: ${packageType}. Must be one of: basic, premium, luxury`);
    }
...
async function saveEstimate({ name, email, phone, propertyName, whatsappUpdates, estimate }) {
    try {
