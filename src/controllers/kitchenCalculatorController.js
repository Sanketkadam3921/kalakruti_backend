    ...
    try {
      ...
      if (!validLayouts.includes(layout)) {
        return res.status(422).json({
          status: "error",
          message:
            "Invalid layout type. Must be one of: l-shaped, u-shaped, straight, parallel",
        });
      }
  ...
  async submitEstimate(req, res, next) {
    try {
      ...
      if (
        ...
        !estimate.layout ||
        !estimate.package ||
        estimate.A === undefined
      ) {
        return res.status(422).json({
          status: "error",
          message:
            "Malformed estimate data. Layout, package, and dimension A are required",
        });
      }
...
const validateEstimate = [
  body("layout")
    ...
    .withMessage(
      "Layout must be one of: l-shaped, u-shaped, straight, parallel"
    ),
  body("A")
    .notEmpty()
...
const validateSubmit = [
  ...
  body("estimate.layout").notEmpty().withMessage("Estimate layout is required"),
  body("estimate.A")
    .notEmpty()
